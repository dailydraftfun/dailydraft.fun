...
export function resultShareText(input: {
  ...
  if (input.result === 'tie') {
    return `My ${input.tier} Pack Duel ended in a tie. Run it back with me.`;
  }

  if (!input.winningPull) {
    return input.result === 'win'
      ? `I won a ${input.tier} Pack Duel. See the verified result.`
      : `I lost a ${input.tier} Pack Duel. Run it back with me.`;
  }

  if (input.result === 'win') {
    return `I won a ${input.tier} Pack Duel with ${input.winningPull.name} at ${input.winningPull.value}.`;
  }

  return `I lost a ${input.tier} Pack Duel to ${input.winningPull.name} at ${input.winningPull.value}. Revenge is one click away.`;
}
