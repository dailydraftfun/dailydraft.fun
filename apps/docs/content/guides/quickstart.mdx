...
description: From pack discovery to a shareable OpenPacks Duel receipt.
---
...
Poll `GET /v1/duels/{duelId}` with the participant wallet-session bearer token,
or consume signed webhooks with an integration credential. Treat a duel status
as final only when its response includes an on-chain signature and the required
confirmation level.
